...
class DocumentNodes:
    ...
    def initalize_sheet(self):
        ...
        sheet_labels = ['Cluster Id', 'Execution_Paths', 'tfidf_word', 'tfidf_method', 'tfidf_word_and_docstring',
                        'tfidf_method_and_docstring', 'lda_word', 'lda_method', 'lsi_word', 'lsi_method', 'tree_context_based_label',
    ...
    def labeling_cluster(self, execution_paths_of_a_cluster, execution_paths_of_siblings, k, v, parent_label):
        ...
        start = timer()
        key_words = self.tree_context_based_label(execution_paths_of_a_cluster, execution_paths_of_siblings,
                                                  parent_label)
        end = timer()
        ...
        return {'key': k, 'parent': v, 'tfidf_word': tfidf_word, 'tfidf_method': tfidf_method,
                ...
                'text_rank': text_rank, 'tree_context_based_label': key_words, 'spm_method': spm_method,
    ...
    def tree_context_based_label(self, execution_paths_of_a_cluster, execution_paths_of_siblings, parent_label):
        """
        Labeling based on the relationship between a node and its siblings by trying to find words that appear by
        finding words that frequently appear in execution paths in a node while not appearing as much in sibling nodes.
        The set of words in an execution path are obtained by going through each method in the execution and adding that
        method's docstring words (with stop words removed and words being lowercased and lemmatised) to the set of words
        but if the docstring doesn't exist, the method's name gets added instead. Set of words in a node is the union of
        the set of words in all the execution paths in the node. Each word in the set of words in a node first gets
        assigned a calculation of the frequency of the word's appearance amongst every execution path in the node's set
        of words. Then that value gets multiplied by 1-x where x is the frequency of the word's appearance amongst all
        sets of words generated by each of the execution paths in the sibling nodes. Then, the 5 words with the highest
        values get chosen as the label though 5-word limit can exceed if there are more than 5 words with a value at 1.
        Note that words must at least have a value of 0.25 but this choice is arbitrary and thus can be modified.
        """
        cluster_word_freq = {}
        sibling_word_freq = {}
